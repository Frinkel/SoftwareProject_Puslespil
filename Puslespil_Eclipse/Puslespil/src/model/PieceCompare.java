...
public class PieceCompare {
	...
	public AngleLength[] createAncleLengthArray(Point2D.Float[] piece) {
		...
		for (int i = 0; i < (listLength - 1); i++) {
			angleLength[i] = new AngleLength(
					getAngleFromThreePoints(piece[i], piece[i + 1], piece[(i + 2) % (listLength - 1)]),
					distance(piece[i], piece[i + 1]));
		}
		return angleLength;
	}
	...
	public float getAngleFromThreePoints(Point2D.Float point1, Point2D.Float point2, Point2D.Float point3) {
		...
		Point2D.Float vectorBC = new Point2D.Float((float) (point3.getX() - point2.getX()),
				(float) (point3.getY() - point2.getY()));
	...
	private float distanceToCenter(Float point1, Float point3) {
		...
		float distance = (float) (Math.abs(((x2 - x1) * (y1 - y0)) - ((x1 - x0) * (y2 - y1)))
