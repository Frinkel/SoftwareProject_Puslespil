...
public class PuzzleSolver {
	...
	public PieceAndAngleDatatype[] solvePuzzle(Object[] pieceList) {
		...
		PieceAndAngleDatatype[] pieceAndRotationalAngle = rotatePiecesOnPath(startingIndex, pieceList.length,
				matchingArray, adjacencyArray, pieceList, pC);
		return pieceAndRotationalAngle;

	}

	private PieceAndAngleDatatype[] rotatePiecesOnPath(int startingIndex, int length, Object[] matchingArray,
			Object[] adjacencyArray, Object[] pieceList, PieceCompare pC) {
		...
		for (int i = 0; i < length - 1; i++) {
			...
			pieceAndRotationalAngle[i + 1] = rotatePiecesAndFindCenter(nextPiece[0], nextPiece[1], matchingArray,
					pieceList, pieceAndRotationalAngle, pC);
	...
	private PieceAndAngleDatatype rotatePiecesAndFindCenter(int piece1, int piece2, Object[] matchingArray,
			Object[] pieceList, PieceAndAngleDatatype[] pieceAndRotationalAngle, PieceCompare pC) {
		...
		if (pair[0] == piece1) {
			Point2D.Float center = getPosOfCenter(threeMatchingPoints,
					angle + (pieceAndRotationalAngle[i].getPieceAngle()), (pieceAndRotationalAngle[i].getPieceAngle()),
					(pieceAndRotationalAngle[i].getCenter()), 1);
			pieceRotationAndCenter = new PieceAndAngleDatatype(piece2,
					angle + (pieceAndRotationalAngle[i].getPieceAngle()), center);
		} else {
			Point2D.Float center = getPosOfCenter(threeMatchingPoints,
					((-angle) + (pieceAndRotationalAngle[i].getPieceAngle())),
					(pieceAndRotationalAngle[i].getPieceAngle()), (pieceAndRotationalAngle[i].getCenter()), -1);
			pieceRotationAndCenter = new PieceAndAngleDatatype(piece2,
					-angle + (pieceAndRotationalAngle[i].getPieceAngle()), center);
		}
	...
	private Point2D.Float getPosOfCenter(Point2D.Float[][] threeMatchingPoints, float angle, float prevAngle,
			Point2D.Float prevCenter, int direction) {
		Point2D.Float[] pointsToRotate1 = new Point2D.Float[] { (threeMatchingPoints[1][0]),
				(threeMatchingPoints[1][1]), (threeMatchingPoints[1][2]) };
		Point2D.Float[] rotatedPoints1 = rotatePoints(pointsToRotate1, angle);

		Point2D.Float[] pointsToRotate2 = new Point2D.Float[] { (threeMatchingPoints[0][0]),
				(threeMatchingPoints[0][1]), (threeMatchingPoints[0][2]) };
		...
		Point2D.Float vectorBA = new Point2D.Float((float) (point1.getX() - point2.getX()),
				(float) (point1.getY() - point2.getY()));
	...
	private int[] getPair(int integer, int integer2, Object[] matchingArray) {
		...
		for (int i = 0; i < matchingArray.length; i++) {
			...
			if ((pairToCheck[0] == integer2 && pairToCheck[1] == integer)
					|| (pairToCheck[1] == integer2 && pairToCheck[0] == integer)) {
				return pairToCheck;
			}
		}
		return new int[] { -1 };
	...
	private int[] getPath(int startingIndex, int[][] adjacencyArray, int[] alreadyCovered, int length) {
	...
	private float findRotationOfMatchingPair(Point2D.Float[][] threeMatchingPoints, PieceCompare pC) {
		Point2D.Float vector0 = new Point2D.Float(
				(float) (((threeMatchingPoints[0][0])).getX() - (threeMatchingPoints[0][2]).getX()),
				(float) ((threeMatchingPoints[0][0]).getY() - (threeMatchingPoints[0][2]).getY()));
		Point2D.Float vector1 = new Point2D.Float(
				(float) ((threeMatchingPoints[1][2]).getX() - (threeMatchingPoints[1][0]).getX()),
				(float) ((threeMatchingPoints[1][2]).getY() - (threeMatchingPoints[1][0]).getY()));
		...
		Point2D.Float[] pointsToRotate = new Point2D.Float[] { (threeMatchingPoints[1][0]), (threeMatchingPoints[1][1]),
				(threeMatchingPoints[1][2]) };
		Point2D.Float[] rotatedPoints = rotatePoints(pointsToRotate, angle);

		float d1 = ((float) (((double) Math.round((pC.distance((threeMatchingPoints[0][2]), rotatedPoints[0])) * 100d))
				/ 100));
		float d2 = ((float) (((double) Math.round((pC.distance((threeMatchingPoints[0][1]), rotatedPoints[1])) * 100d))
				/ 100));
		float d3 = ((float) (((double) Math.round((pC.distance((threeMatchingPoints[0][0]), rotatedPoints[2])) * 100d))
				/ 100));
	...
	private Point2D.Float[] rotatePoints(Point2D.Float[] pointsToRotate, float angle) {
		...
		for (int i = 0; i < 3; i++) {
			float rotatedX = (float) (PApplet.cos(angle) * (pointsToRotate[i].getX())
					- (PApplet.sin(angle) * (pointsToRotate[i].getY())));
			float rotatedY = (float) (PApplet.sin(angle) * (pointsToRotate[i].getX())
					+ (PApplet.cos(angle) * (pointsToRotate[i].getY())));
